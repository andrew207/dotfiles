...
apply_hyprland() {
    ...
    # Hyprland regenerates an example stub hyprland.conf when the file is
    # absent; it loads after hyprland.lua and clobbers the Lua settings.
    # Keep a comment-only pointer instead so nothing overrides the Lua config.
    local conf="$HOME/.config/hypr/hyprland.conf"
    if [[ ! -e "$conf" ]] || grep -q 'STUB\|autogenerated' "$conf" 2>/dev/null; then
        if [[ $DRY_RUN -eq 1 ]]; then
            log "[dry-run] write  pointer $conf (replaces generated stub)"
        else
            cat > "$conf" <<'EOF'
# The real config lives in hyprland.lua (Lua entry point). It requires:
#   env.lua, execs.lua, general.lua, colors.lua, rules.lua, keybinds.lua,
#   monitors.lua (+ optional custom/*.lua overrides)
# This file is a comment-only pointer: it must set no options, otherwise it
# loads after the Lua config and clobbers its settings. If deleted, Hyprland
# regenerates an example stub here that overrides hyprland.lua.
EOF
            chmod 644 "$conf"
            log "wrote    pointer $conf"
        fi
    else
        log "kept     $conf (already a non-stub file)"
    fi
